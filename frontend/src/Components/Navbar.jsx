...
console.log(googleCreds)
  useEffect(() => {
    const fetchUserProfile = async () => {
      try {
        const response = await fetch(`http://localhost:8080/accounts_data/${googleCreds.sub}`);
  ...
  const reportListSafe = reportList || [];
  return (
    ...
    <div className={`${isExpanded ? 'w-3/4' : 'w-1/3'} fixed left-0 top-0 bottom-0 bg-gray-800 text-white transition-width duration-300 overflow-hidden`}>
       <div className="flex flex-col items-center h-full">
           ...
           <div className="mt-4 mb-2 margin-10 flex items-center">
             ...
             {isExpanded && profile && (
               <div className="ml-4 text-left">
                 <div className="text-lg font-bold">{profile ? profile.usename : 'Loading'}</div>
                 <div>{profile ? profile.rank : 'Loading'}</div>
                 <div>{profile ? profile.usename : 'Loading'}</div>
               </div>
             )}
           </div>
         ...
         <div className='flex-grow overflow-y-auto w-full'>
           ...
           {reportListSafe.map((report, index) => (
